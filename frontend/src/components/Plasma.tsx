...
void mainImage(out vec4 o, vec2 C) {
  ...
  float i = 0., d, z, T = iTime * uSpeed * uDirection;
  vec3 O = vec3(0.), p, S;
  o = vec4(0.);
  ...
  for (vec2 r = iResolution.xy, Q; i < 30.; i++) {
    ...
    p.x += .4*(1.+p.y)*sin(d*2.0 + p.x*0.4)*cos(.6*d + p.x*0.2); 
    p.xz *= mat2(cos(p.y*1.5+vec4(0,11,33,0)-T*1.2));
    Q = p.xz;
    z+= d = abs(sqrt(length(Q*Q)) - .25*(5.+S.y))/3.+8e-4; 
    o = 1.+sin(S.y*1.5+p.z*.8+S.z-length(S-p)+vec4(2,1,0,8));
    O += o.w/d*o.xyz;
  }
